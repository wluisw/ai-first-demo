...
description: 任意功能变更、bug修复、接口改动完成后，生成PR前必须在对应服务的 docs/services/<英文可读名>/CHANGELOG.md 追加条目——keep-a-changelog格式，按服务目录分隔。
...
2. **按服务隔离** — `docs/services/<英文可读名>/CHANGELOG.md`（目录用英文可读名，如 "Order Entry"；kebab→可读名映射见 CLAUDE.md「目录结构」节），多服务变更分别写
